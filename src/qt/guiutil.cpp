...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(!uri.isValid() || uri.scheme() != QString("billionaire"))
        return false;
...
bool parseBitcoinURI(QString uri, SendCoinsRecipient *out)
{
    ...
    if(uri.startsWith("billionaire://", Qt::CaseInsensitive))
    {
        uri.replace(0, 11, "billionaire:");
    }
...
{
    QString ret = QString("billionaire:%1").arg(info.address);
    int paramCount = 0;
...
fs::path static GetAutostartFilePath()
{
    std::string chain = ChainNameFromCommandLine();
    if (chain == CBaseChainParams::MAIN)
        return GetAutostartDir() / "billionaire.desktop";
    return GetAutostartDir() / strprintf("billionaire-%s.lnk", chain);
}
